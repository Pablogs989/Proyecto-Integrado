    ...
    class Pedido
    {
        ...
        public static List<LineaPedido> GetLineas(MySqlConnection conexion, int nPed)
        {
            ...
            try
            {
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        Tipo tipo = new Tipo(reader.GetInt32(8), reader.GetString(9));
                        Producto producto = new Producto(reader.GetInt32(4), reader.GetString(6), reader.GetDouble(7), tipo);
                        lineas.Add(new LineaPedido(reader.GetInt32(3), nPed, reader.GetInt32(0), producto));
                    }
                }
            }
            catch (MySqlException ex)
        ...
        public static void AgregarPedido(MySqlConnection conexion, Pedido pedido)
        {
            ...
            try
            {
                ...
                {
                    string insertLinea = string.Format("INSERT INTO lineapedido VALUES (NULL, {0}, {1}, {2});", GetUltNumPed(conexion), linea.Producto.Id, linea.Cant);
        ...
        //SI ESTÁN PAGADOS SOLOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO CAPULLO
        public static double GetCaja(MySqlConnection conexion, DateTime fecha)
        {
            string consulta = string.Format("SELECT numPedido FROM pedidos WHERE fecha = '{0}'", fecha.ToString("yyyy/MM/dd"));
            MySqlCommand comando = new MySqlCommand(consulta, conexion);
            double caja = 0;
            try
            {
